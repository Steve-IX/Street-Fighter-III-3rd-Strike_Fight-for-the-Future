...
Default Player 1 controls are `Shift` for Coin, `Enter` for Start, arrow keys for movement, `X/S/Z` for punches, and `A/Q/E` for kicks. Click **Focus game** or the game screen if the page chrome has keyboard focus.
